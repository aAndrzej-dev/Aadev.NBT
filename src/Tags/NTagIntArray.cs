...
namespace Aadev.NBT.Tags
{
    ...
    public class NTagIntArray : NTag, INTagArray
    {
        private readonly IList<NTagInt> children = new List<NTagInt>();

        /// <summary>
        /// Type of each child element in array
        /// </summary>
        public NTagType ChildType => NTagType.Int;
        public override NTagType Type => NTagType.IntArray;
        ...
        public void AddChild(NTag nTag)
        {
            ...
            {
                throw new InvalidCastException($"{nameof(nTag)} is not {typeof(NTagInt).Name}");
            }
        }
        ...
        internal override void ToByteArray(ref int offset, Span<byte> bytes, Endianness endianness)
        {
            ...
            foreach (NTagInt item in children)
            {
